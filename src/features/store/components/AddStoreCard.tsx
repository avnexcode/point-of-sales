...
export const AddStoreCard = ({ className, onAddStore }: AddStoreCardProps) => {
  const { t } = useTranslation();
  return (
    <Link href={"/dashboard/store/create"} className="w-full max-w-xs">
      <Card
        className={cn(
        ...
        <CardContent className="flex h-full min-h-[320px] flex-col items-center justify-center space-y-4 p-8">
          ...
          <div className="space-y-2 text-center">
            <h3 className="text-foreground text-lg font-semibold">
              {t("components.store.addStoreCard.title")}
            </h3>
            <p className="text-muted-foreground max-w-[200px] text-sm">
              {t("components.store.addStoreCard.caption")}
            </p>
          </div>
