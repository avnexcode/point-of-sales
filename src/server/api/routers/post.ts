...
import { createProductRequest, queryParams } from "@/server/validations";
...
export const postRouter = createTRPCRouter({
  create: publicProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        const product = { ...input };
        console.log({ fromServer: serverI18n.t("home.title") });
        return product;
      } catch (error) {
  ...
  getProducts: publicProcedure.input(queryParams).query(async () => {
    const { t } = serverI18n;
    console.log({ date: formatDate(new Date(), "full", t) });
    console.log({
      fromServer: String(
        t("home.title", {
